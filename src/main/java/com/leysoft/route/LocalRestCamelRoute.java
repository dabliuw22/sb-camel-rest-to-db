...
public class LocalRestCamelRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
        onException(CountryException.class)
                .log(LoggingLevel.ERROR, "Error: CountryException ${body}").process(mailProcessor);
        restConfiguration().component("jetty").host("localhost").port(8081);
        rest().post(fromLocalRest).to(direct);
        from(direct).routeId("localRestCamelRoute").log("body -> ${body}")
                .convertBodyTo(String.class).process(countryProcessor).process(databaseProcessor)
                .to(toDb);
    }
}
